bool check_collision ( int ** falling_tetris, int ** middle_tetris, const int falling_coordinates[2], const int falling_width, const int falling_height,
  const int middle_coordinates[2], const int middle_width, const int middle_height)
{
  // decide the four boundaries of smaller and larger tetris
  int smaller_left = (middle_width > falling_width) ? falling_coordinates[0] : middle_coordinates[0];
  int larger_left = (middle_width > falling_width) ? middle_coordinates[0] : falling_coordinates[0];
  int smaller_right = (middle_width > falling_width) ? (falling_coordinates[0] + falling_width - 1) : (middle_coordinates[0] + middle_width - 1);
  int larger_right = (middle_width > falling_width) ? (middle_coordinates[0] + middle_width - 1) : (falling_coordinates[0] + falling_width - 1);
  int smaller_upper = (middle_height > falling_height) ? falling_coordinates[1] : middle_coordinates[1];
  int larger_upper = (middle_height > falling_height) ? middle_coordinates[1] : falling_coordinates[1];
  int smaller_lower = (middle_height > falling_height) ? (falling_coordinates[1] + falling_height - 1) : (middle_coordinates[1] + middle_height - 1);
  int larger_lower = (middle_height > falling_height) ? (middle_coordinates[1] + middle_height - 1) : (falling_coordinates[1] + falling_height - 1);
  ...
  if (overlapping_upper != -1 && overlapping_left != -1) {
    for (int i = overlapping_upper; i <= overlapping_lower; i++) {
      for (int j = overlapping_left; j <= overlapping_right; j++) {
        if (falling_tetris[i - falling_coordinates[0]][j - falling_coordinates[1]] == 1 && middle_tetris[i - middle_coordinates[0]][j - middle_coordinates[1]] == 1) {
          flag = 1;
          break;
        }
      }
