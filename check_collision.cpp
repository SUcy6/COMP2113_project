...
// check collision
bool check_collision ( ftetris* fp, mtetris mp, int ** middle_tetris)
{
  ...
  int smaller_upper = (mp.H > fp->H) ? next_coordinates[1] : mp.y;
  ...
  if (overlapping_upper != -1 && overlapping_left != -1) {
    for (int i = overlapping_upper; i <= overlapping_lower; i++) {
      for (int j = overlapping_left; j <= overlapping_right; j++) {
        if ((fp)->shape[i - next_coordinates[1]][j - next_coordinates[0]] == 1
        && middle_tetris[i - mp.y][j - mp.x] == 1) {
          flag = 1;
          break;
        }
      }
