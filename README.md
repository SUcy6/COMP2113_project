...
- Players control the middle active piece so that the tile will collide falling teris and combine into a new teris. Press `'w'`, `'a'`, `'s'`, `'d'` to move the active piece `up`, `left`, `down`, `right`. Press `'j'`, `'k'` to rotate the teris `anti-clockwise` or `clockwise`. Press `'q'` to quit the game and output a file of game status.
